...
export async function generateTTS(
  ...
  switch (config.providerId) {
    ...
    case 'silero-tts':
      return await generateSileroTTS(config, text);
...
/**
 * Silero TTS implementation (via CallMind API on Management Console)
 */
async function generateSileroTTS(
  config: TTSModelConfig,
  text: string,
): Promise<TTSGenerationResult> {
  const baseUrl = config.baseUrl || TTS_PROVIDERS['silero-tts'].defaultBaseUrl;

  // CallMind TTS uses X-CallMind-Key header for auth
  const response = await fetch(`${baseUrl}/synthesize/stream`, {
    method: 'POST',
    headers: {
      'X-CallMind-Key': config.apiKey!,
      'Content-Type': 'application/json; charset=utf-8',
    },
    body: JSON.stringify({
      text,
      voice: config.voice || 'xenia',
      sample_rate: 48000,
      format: 'mp3',
      use_ssml: true,
      rate: Math.round((config.speed || 1.0) * 95),
      pitch: -2,
    }),
  });
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => response.statusText);
    throw new Error(`Silero TTS API error: ${errorText}`);
  }
