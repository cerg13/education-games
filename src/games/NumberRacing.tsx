...
const defaultState = { stars: 0, unlockedCars: ['starter'], selectedCar: 'starter', stats: { totalRaces: 0, totalCorrect: 0, totalWrong: 0, bestStreak: 0 }, difficulty: 1, musicOn: true, soundOn: true };
const MAX_ERRORS = 5;

export default function NumberRacing() {
  ...
  const generateLocalTask = useCallback((diff) => {
    const types = ['compare', 'bigger', 'smaller', 'sequence', 'count'];
    if (diff >= 3) types.push('addSub');
    const type = types[Math.floor(Math.random() * types.length)];
    const maxNum = Math.min(4 + diff, 10);
    ...
    switch(type) {
      case 'compare': {
        ...
        while (n2 === n1) n2 = Math.floor(Math.random() * maxNum) + 1;
      ...
      case 'sequence': {
        const start = Math.floor(Math.random() * Math.max(1, maxNum - 4)) + 1;
      ...
      case 'addSub': {
        ...
        const b = Math.floor(Math.random() * Math.min(4, a)) + 1;
        const isAdd = Math.random() > 0.5;
        const left = isAdd ? a : a + b;
        const right = b;
        const result = isAdd ? a + b : a;
        visual = { type: 'addSub', left, right, op: isAdd ? '+' : '-' };
        const addOpts = [result];
        if (result > 0) addOpts.push(result - 1);
        addOpts.push(result + 1);
        options = [...new Set(addOpts)].filter(n => n >= 0 && n <= 10).slice(0, 3).sort(() => Math.random() - 0.5).map(n => ({ value: n, display: n }));
        correct = result;
        break;
      }
    }
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (result?.value) setGameState({...defaultState, ...JSON.parse(result.value)});
      } catch(e) {}
      setDataLoaded(true);
    };
    loadData();
  ...
  useEffect(() => {
    if (screen === 'race' && gameState.soundOn && isMoving) {
  ...
  }, [screen, gameState.soundOn, isMoving, sound]);

  // Music control
  useEffect(() => {
    if (screen === 'race' && gameState.musicOn) {
      sound.startMusic();
    } else {
      sound.stopMusic();
    }
    return () => sound.stopMusic();
  }, [screen, gameState.musicOn]);

  const saveData = async (data) => {
    try { await window.storage.set('numberRacingData', JSON.stringify(data)); } catch(e) {}
  };
  ...
  const handleAnswer = (answer) => {
    ...
    if(isCorrect) {
      ...
      setStreak(s => s + 1);
      if(streak >= 2 && gameState.difficulty < 5) updateGameState({ difficulty: gameState.difficulty + 1 });
      ...
      if(streak <= -2 && gameState.difficulty > 1) updateGameState({ difficulty: gameState.difficulty - 1 });
    }
  ...
  if(screen === 'race') return (
    <div className="min-h-screen bg-gradient-to-b from-sky-400 via-cyan-400 to-emerald-400 p-3 flex flex-col relative overflow-hidden">
      <style>{styles}</style>
      <StarBurst show={showStarBurst} />
      <SpeedLines active={isMoving && !showResult} />
      <FinishRibbon show={showFinish} />
      
      {/* Animated road */}
      <div className="absolute inset-0 opacity-20" style={{background:'repeating-linear-gradient(0deg,transparent,transparent 25px,white 25px,white 30px)',animation: isMoving ? 'roadMove 0.4s linear infinite' : 'none'}}/>
      
      {/* Trees/scenery passing by */}
      <div className="absolute left-2 top-0 bottom-0 w-8 overflow-hidden opacity-40">
        {[...Array(5)].map((_,i) => <div key={i} className="absolute text-2xl" style={{top:`${i*25}%`,animation: isMoving ? `roadMove ${0.8+i*0.1}s linear infinite` : 'none'}}>🌲</div>)}
      </div>
      <div className="absolute right-2 top-0 bottom-0 w-8 overflow-hidden opacity-40">
        {[...Array(5)].map((_,i) => <div key={i} className="absolute text-2xl" style={{top:`${i*25+10}%`,animation: isMoving ? `roadMove ${0.9+i*0.1}s linear infinite` : 'none'}}>🌳</div>)}
      </div>
        ...
        <div className={`bg-white/95 backdrop-blur rounded-3xl p-4 shadow-2xl w-full max-w-sm transition-all ${showResult === 'correct' ? 'ring-8 ring-green-400' : showResult === 'wrong' ? 'ring-8 ring-red-400' : ''}`} style={{animation: showResult === 'correct' ? 'pop 0.3s' : showResult === 'wrong' ? 'shake 0.4s' : 'none'}}>
          {showResult ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2">{task.options.map((opt,i) => <button key={i} onClick={() => handleAnswer(opt.value)} className="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white text-2xl font-black py-4 rounded-xl shadow-xl hover:scale-110 active:scale-95 transition-all" style={{animation:'pulse 2s infinite',animationDelay:`${i*0.2}s`}}>{opt.display}</button>)}</div>
            </>
          )}
        </div>
      </div>
  ...
  if(screen === 'failed') return (
    <div className="min-h-screen bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800 p-4 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="space-y-3 w-full max-w-xs">
        ...
        <button onClick={() => { if(gameState.soundOn) sound.playClick(); setScreen('menu'); }} className="w-full bg-white/20 text-white py-4 rounded-2xl